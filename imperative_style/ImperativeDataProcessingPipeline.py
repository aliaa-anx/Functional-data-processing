...
dfn["Transaction Date"] = pd.to_datetime(df["Transaction Date"], errors="coerce")
dfn["Year"] = dfn["Transaction Date"].dt.year
total_sales_per_year = dfn.groupby("Year")["Total Spent"].sum().reset_index()
total_sales_per_year["Total Spent"] = total_sales_per_year["Total Spent"].round(2)
...
for col in columns:
    ...
    variance_value = round(df[col].var(), 2)
